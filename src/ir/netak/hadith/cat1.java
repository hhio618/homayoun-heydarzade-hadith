...
public class cat1 extends MyCustomListActivity {
    ...
    private void setContent() {
        ...
        Cursor curser=db.rawQuery("SELECT * FROM \"etic\" where cat="+"\""+cat+"\" order by cat1 ASC", null);
        curser.moveToFirst();
        String temp = "nothing";
        int i=0;
        if(curser!=null){
        	do {
        		...
        		if(temp.compareTo(curser.getString(6))!=0)
        		{
                  value.add(curser.getString(6));
                    i++;
        		map.put("head","");
                map.put("text",farsi(curser.getString(6)));
                map.put("icon", R.drawable.tazhib);
        		mylist.add(map);
        		}
  		

        		temp=curser.getString(6);
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        intent.putExtra("cat", cat);
         intent.putExtra("cat1", value.get(position));
         startActivity(intent);
        }
@Override
