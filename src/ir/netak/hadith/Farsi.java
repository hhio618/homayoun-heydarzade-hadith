...
public final class Farsi {
	...
	private final static String szLamAndAlef = Character
			.toString((char) 0xfedf) + Character.toString((char) 0xfe8e); // Lam
																			// +
																			// Alef

	private final static String szLamStickAndAlef = Character
			.toString((char) 0xfee0) + Character.toString((char) 0xfe8e); // Lam
																			// (Sticky
																			// !!!)+
																			// Alef
	...
	private final static String szLamAndAlefWoosim = Character
			.toString((char) 0xe1) + Character.toString((char) 0xbb); // Lam +
																		// Alef

	private final static String szLamStickAndAlefWoosim = Character
			.toString((char) 0x90) + Character.toString((char) 0xbb); // Lam
																		// (Sticky
																		// !!!)+
																		// Alef
	...
	public static boolean isErab(char chIn) {

		if (chIn >= (char) 0x064b && chIn <= (char) 0x065e) {
			return true;
		}
	...
	private static boolean isEnd(int length, int currentIndex) {
			...
			for (int i = 0; i < In.length(); i++) {
						...
						|| (ch == (char) 0x06c0)) // is an Arabic character?
				{
					...
					if (i == In.length() - 1)
						...
						linkAfter = false;
						for (int index = i + 1; index < In.length(); index++) {
							if (!isErab(chIn[index])) {
								linkAfter = (isFromTheSet1(chIn[index]) || isFromTheSet2(chIn[index]));
								break;
							}
						}
						/*
						 * if((chIn[i + 1] >= (char)0x064b && chIn[i + 1] <=
						 * (char) 0x065e)) if(i == In.length() - 2) linkAfter =
						 * false; else linkAfter = (isFromTheSet1(chIn[i + 2])
						 * || isFromTheSet2(chIn[i + 2])); else linkAfter =
						 * (isFromTheSet1(chIn[i + 1]) || isFromTheSet2(chIn[i +
						 * 1]));
						 */
					}
					if (i == 0)
						...
						if ((chIn[i - 1] >= (char) 0x064b && chIn[i - 1] <= (char) 0x065e)) {
							if (i - 2 < 0)
								linkBefore = false;
							else
								linkBefore = isFromTheSet1(chIn[i - 2]);
						} else
